...
jQuery(document).ready(function ($) {
    ...
    //get list compare items
    /*
    $.get("/ajax/list_compare.php",
        function(data) {
            try {
                var compareItems = $.parseJSON(data);
                var detailCompare = $(".compare-item-control_add");
                $("#top-compare-cnt").text(compareItems.length);
                $.each(compareItems, function(k, itemId) {
                    var item = $('#compare_item_' + itemId);
                    if (item.length)
                        item.addClass('act');

                });
                if(detailCompare.length)
                    detailCompare.show();
            } catch (e) {
    ...
    */
    ...
    $(document).on("click", '.compare-item-control_add', function () {
        var id = $(this).parent().data('id');
